...
import { uploadGoodsAPI } from '@/apis/goods';
...
const uploadGoods = async () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      await uploadGoodsAPI(form.value);
      ElMessage({ type: "success", message: "商品上传成功" });
      router.replace({ path: "/" });
    }
  });
};
  ...
  <div class="form">
    ...
    >
      ...
      <div
        ...
        "
      >
        <el-button size="large" class="subBtn" @click="uploadGoods"
          >上传商品</el-button
        >
      </div>
    </el-form>
